...
# testing which sumpool we have loaded changing if needed
# model.sumpool.pool_mask = torch.ones((5,1,9,9))
# model.sumpool.conv.weight = torch.nn.Parameter(torch.ones((5,1,9,9)).to(config["device"]),requires_grad=False)
# print('Updating sum pool mask (post-training)!',model.sumpool.pool_mask.shape)
