...
ax.errorbar(bin_centers,frac_tar_match,xerr=bin_width/2,yerr=match_tar_error,color='cyan',marker='.',ms=5.5,elinewidth=1.2,ls='none',label=f'% Matched Target Boxes (Accuracy)')
...
f,ax = plt.subplots(1,1,figsize=(8, 6))
ax.errorbar(bin_centers,frac_tar_dRmatch,xerr=bin_width/2,yerr=dRmatch_tar_error,color='cyan',marker='.',ms=5.5,elinewidth=1.2,ls='none',label=f'% deltaR Matched Anti-kt jets (Accuracy)')
ax.errorbar(bin_centers,frac_p_dRunmatch,xerr=bin_width/2,yerr=dRunmatch_p_error,color='coral',marker='x',ms=5.5,elinewidth=1.2,ls='none',label=f'% deltaR Unmatched CNN jets (Fake rate)')
ax.axhline(y=1.0,color='silver',ls='--',alpha=0.7)
ax.axhline(y=0.0,color='silver',ls='--',alpha=0.7)
ax.set(xlabel='Transverse Momentum (GeV)',ylabel=f'Fraction of jets')
ax.set_ylim((0.0,1.3))
ax.legend(loc='lower left',bbox_to_anchor=(0.3, 0.45),fontsize="small")
hep.atlas.label(ax=ax,label='Work in Progress',data=False,lumi=None,loc=1)
...
f,ax = plt.subplots(1,1,figsize=(8, 6))
ax.errorbar(bin_centers,frac_tar_dRmatch,xerr=bin_width/2,yerr=dRmatch_tar_error,color='cyan',marker='.',ms=5.5,elinewidth=1.2,ls='none',label=f'% Matched Target Boxes (Accuracy)')
...
for bin_idx in range(len(bin_edges)-1):
    ...
    print(f"There are {len(dRtruthmatched_p_bin_i)+len(dRtruthunmatched_p_bin_i)} predicted jets and {len(dRtruthmatched_tru_bin_i)+len(dRtruthunmatched_tru_bin_i)} target jets in bin {bin_idx} ([{bin_edges[bin_idx]}, {bin_edges[bin_idx+1]}])")

    # Finally, get the fraction (un)matched in each bin
    print(f"Fraction of predictions that are matched: {len(dRtruthmatched_p_bin_i)/(len(dRtruthmatched_p_bin_i)+len(dRtruthunmatched_p_bin_i)):.3f}\t unmatched: {len(dRtruthunmatched_p_bin_i)/(len(dRtruthmatched_p_bin_i)+len(dRtruthunmatched_p_bin_i)):.3f}")
    print(f"Fraction of targets that are matched: {len(dRtruthmatched_tru_bin_i)/(len(dRtruthmatched_tru_bin_i)+len(dRtruthunmatched_tru_bin_i)):.3f}\t unmatched: {len(dRtruthunmatched_tru_bin_i)/(len(dRtruthmatched_tru_bin_i)+len(dRtruthunmatched_tru_bin_i)):.3f}\n")
...
f,ax = plt.subplots(1,1,figsize=(8, 6))
ax.errorbar(bin_centers,frac_tru_dRtruthmatch,xerr=bin_width/2,yerr=dRtruthmatch_tru_error,color='gold',marker='.',ms=5.5,elinewidth=1.2,ls='none',label=f'% Matched Truth Boxes (Accuracy)')
