...
parser.add_argument('-te','--trained_epochs', type=int, required=True, help='Number of epochs completed previously',)
...
config = {
    ...
    "trained_epochs"   : int(args.trained_epochs),
...
print("Loading trained model as startting point, continuing training from ",config["trained_epochs"]," for ",config["n_epochs"]," epochs")
model_name = "jetSSD_{}_{}e".format(model.backbone_name,config["trained_epochs"])
...
print(f'Resuming training from epoch {config["trained_epochs"]}...')
...
# ###
    ...
    print(f"\tEpoch {config["trained_epochs"]+epoch} / {config['n_epochs']}: train loss {mean(running_loss):.4f}, train time {time.perf_counter() - beginning:.2f}s, LR: {optimizer.param_groups[0]['lr']:.4f}")
    ...
    print(f"\tEpoch {config["trained_epochs"]+epoch} / {config['n_epochs']}: valid loss {mean(running_val_loss):.4f}, valid time {time.perf_counter() - val_beginning:.2f}s")
...
# save trained model
model_name = "jetSSD_{}_{}e".format(model.backbone_name,config["trained_epochs"]+config["n_epochs"])
print(f'Saving updated model now at new location...\t{model_name}')
