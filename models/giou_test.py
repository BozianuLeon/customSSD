...
if __name__ == '__main__':
    ...
    pred_boxes = torch.tensor([
        [[10., 10., 20., 20.],  # box1 (x1, y1, x2, y2)
        [30., 30., 40., 40.]], # box2
        [[15., 15., 25., 25.],  # box1
        [35., 35., 45., 45.]]  # box2
    ], dtype=torch.float32)

    # Create target boxes (batch_size=2, boxes=2)
    ...
    # Calculate the loss
    loss = tv.ops.generalized_box_iou_loss(pred_boxes, target_boxes)
    print(f"GIoU Loss:\n{loss}")
    iouloss = IouLoss(ltype='GIoU')
    loss = iouloss(pred_boxes, target_boxes)
    print(f"GiOu Loss:\n{loss}")
    loss = tv.ops.distance_box_iou_loss(pred_boxes, target_boxes)
    print(f"DIoU Loss:\n{loss}")
    iouloss = IouLoss(ltype='DIoU')
    loss = iouloss(pred_boxes, target_boxes)
    print(f"DiOu Loss:\n{loss}")
    loss = tv.ops.complete_box_iou_loss(pred_boxes, target_boxes)
    print(f"CIoU Loss:\n{loss}")
    iouloss = IouLoss(ltype='CIoU')
    loss = iouloss(pred_boxes, target_boxes)
    print(f"CiOu Loss:\n{loss}")
    ...
    torch.manual_seed(0)
    iouloss = IouLoss(ltype='GIoU').cuda()
    print(iouloss)
    giou_loss = tv.ops.generalized_box_iou_loss
    print(giou_loss)
    ...
    for i in range(5):
        ...
        tvloss = giou_loss(pred, tar)
        # tvloss.sum().backward(retain_graph=True)
        # print(origin2.grad)

        print((loss.detach().cpu().numpy()))
        print((tvloss.detach().cpu().numpy)())
        print()
    print()
    print(iouloss)
    print(giou_loss)