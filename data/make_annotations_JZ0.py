...
def get_jet_bounding_boxes(jet_data,event_no,extent,min_max_tuple):
    ...
    filtered_pt_jets = jets[jets['AntiKt4EMTopoJets_JetConstitScaleMomentum_pt'] > 15_000] # Select the pt threshold (in MeV)
...
def get_truth_jets(jet_data,event_no,extent,min_max_tuple):
    ...
    # filter jet pt and eta
    filtered_pt_jets = jets[jets['AntiKt4TruthJets_pt'] > 0] # Select the pt threshold (in MeV)
...
if __name__=="__main__":
    ...
    with open(args.output_json+"anns_central_jets_truth_JZ0_test.json",'w') as json_file:
